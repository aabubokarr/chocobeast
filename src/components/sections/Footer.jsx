      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-10 pb-12 border-b border-brand-cream/10">
          ...
          <div className="lg:col-span-4 flex flex-col items-start text-left space-y-4">
            <a href="#" className="flex items-center gap-2 font-serif text-2xl font-black tracking-tight group">
              <motion.div
                whileHover={{ rotate: 15, scale: 1.1 }}
                className="text-brand-plum"
              >
                <Flame className="w-8 h-8 fill-brand-caramel stroke-brand-cream stroke-2" />
              </motion.div>
              <span>
            ...
            <p className="font-sans text-sm text-brand-cream/70 leading-relaxed max-w-sm">
              Artisanal premium confections for wild cravings. Handcrafted from Madagascan heirloom cocoa, conched for 72 hours, and wrapped in 100% compostable sheets.
            </p>
              ...
              {socialLinks.map((social, idx) => {
                ...
                return (
                  <motion.a
                    key={idx}
                    href={social.href}
                    whileHover={{ y: -4, scale: 1.1, backgroundColor: "rgba(217, 165, 92, 0.25)", color: "#F4C77C" }}
          ...
          <div className="lg:col-span-2 flex flex-col items-start text-left space-y-4">
            <h4 className="font-serif text-base font-extrabold text-brand-gold uppercase tracking-wider">Shop</h4>
            <ul className="space-y-2.5">
              {column1.map((link, idx) => (
                <li key={idx}>
                  <a href={link.href} className="font-sans text-sm text-brand-cream/70 hover:text-brand-caramel transition-colors">
                    {link.name}
                  </a>
                </li>
              ))}
          ...
          <div className="lg:col-span-3 flex flex-col items-start text-left space-y-4">
            <h4 className="font-serif text-base font-extrabold text-brand-gold uppercase tracking-wider">Mission</h4>
            <ul className="space-y-2.5">
          ...
          <div className="lg:col-span-3 flex flex-col items-start text-left space-y-4">
            <h4 className="font-serif text-base font-extrabold text-brand-gold uppercase tracking-wider">Support</h4>
            <ul className="space-y-2.5">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center pt-8 text-center text-xs text-brand-cream/50 space-y-4 sm:space-y-0">
          <div>
            <p>© 2026 Chocobeast Confections Ltd. All rights reserved.</p>
          ...
          <motion.button
            ...
            whileHover={{ y: -5, boxShadow: "0 0 15px rgba(244, 199, 124, 0.4)" }}
